...
```
## [CSDVectorDetector.bambda](https://github.com/PortSwigger/bambdas/blob/main/CustomAction/CSDVectorDetector.bambda)
### Detects potential client-side desync (CSD) vectors by sending an HTTP/1.1 POST probe with a Content-Length inflated by 1 byte beyond the actual body. An immediate response before the phantom byte is read may indicate a CSD vector.
